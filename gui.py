...
def parseTextToString(diki_word):
  ...
  if diki_word is not None:
    for it, tlumaczenie in enumerate(diki_word["tlumaczenia"]):
      output.append([(",".join(tlumaczenie["znaczenie"])).strip(), True if it < 1 else False])  # + "\n"
  else:
...
#==============ELEMENTY============
  ...
  word_box = CEntry(border, justify = 'center', font=("Arial 17 bold"),borderwidth=0, bg = '#C8A2C8', readonlybackground ='#BDD0e7', selectbackground = '#d3d3d3',selectforeground = 'black', cursor = 'dot')
  word_box.insert(tk.END, str(diki_word['word']))
  word_box['state']='readonly'
  word_box.pack(ipady=3, fill=tk.BOTH, side=tk.TOP, expand=True)

  translation_box = tk.Text(border, font=("Arial 12"),borderwidth=0, bg = '#FEFEF4', selectbackground = '#d3d3d3',selectforeground = 'black', cursor = 'dot')
